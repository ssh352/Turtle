...
```{r}
date <- "2017-04-07"
stats <- read.csv(paste("/media/sjaoraid/strat/Turtle/opt/DonchianOpt",date,".csv", sep=""))
stats <- stats[,-1]
...
```{r}
hist(subset(stats,atr>1)$Net.Trading.PL,breaks =50,xlim = c(-100000,100000), main = paste("Average = ", mean(subset(stats,atr>1)$Net.Trading.PL)))
```
...
for(sym in symbols){
  print(qplot(data = stats[stats$Symbol==sym,],x=atr,y=Num.Trades,facets = .~DonchBreak,color=DonchStop,main=paste("Variation of No. of Trades with Donch Channel Length & ATR for ",sym,sep="")))
...
for(sym in symbols){
  ...
  print(qplot(data = stats[stats$Symbol==sym,],x=atr,y=Net.Trading.PL,facets = .~DonchBreak,color=DonchStop,main=paste("Variation of Net PL with Donch & ATR for ",sym,sep="")))
...
for(sym in symbols){
  ...
  print(qplot(data = stats[stats$Symbol==sym,],x=atr,y=Profit.To.Max.Draw,facets = .~DonchBreak,color=DonchStop,main=paste("Variation of Profit:Maximum Drawdown with Donch & ATR for ",sym,sep="")))
...
for(sym in symbols){
  ...
  print(qplot(data = stats[stats$Symbol==sym,],x=atr,y=Ann.Sharpe,facets = .~DonchBreak,color=DonchStop,main=paste("Variation of Sharpe Ratio with Donch & ATR for ",sym,sep=""),ylim = c(-10,10)))
}
```
